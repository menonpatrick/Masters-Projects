...
public class sjf_simulation {
		...
		public static void arrival(int p, double a, double b) {
			...
			completion(process_id[p]); // completion function called
			...
			for(int i=1;i<10;i++) {
				for(int j=i+1;j<=10;j++) {
					if(burst_time[i]>burst_time[j]) {
						...
						process_id[j] = temp1;
						// sorting the equivalent arrival time array
						temp = arrival_time[i];
		...
		public static void main (String z[]) {
			...
			System.out.println("Shortest Job First (Simulation) :");
			System.out.println();
			...
			for(i=1;i<=10;i++) {
				arrival(process_id[i],arrival_time[i],burst_time[i]); // calling the arrival function
